...
Код Xamarin.Forms выполняется на нескольких платформах, каждая из которых имеет свою собственную файловую систему. Ранее это означало, что чтение и запись файлов проще всего выполнялось с помощью собственных API файлов на каждой платформе. Кроме того внедренные ресурсы, более простое решение для распространения файлов данных с помощью приложения. Тем не менее с помощью .NET Standard 2.0 имеется возможность совместного использования кода доступа к файлу в библиотеки .NET Standard.
