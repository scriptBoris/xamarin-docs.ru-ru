...
Эти требования подразумевают, что видеопроигрыватель необходимо сделать доступными текущее состояние, указывающее, при его воспроизведения или приостановлено, или если он еще не готов для воспроизведения видео. (Также поддерживает каждая платформа свойствами, указывающими, если видео могут быть приостановлены, также можно переместить в новую позицию, но эти свойства применяются для потоковой передачи видео, а не видеофайлы, поэтому они не поддерживаются в `VideoPlayer` описано здесь.)
...
Эта возможность обеспечивается каждой платформы, за исключением того, что это обычный черный треугольник, который не похож объемный внешний вид **Пауза** и **остановить**. Один из вариантов, выполните 0x25B6 кодовых точек с использованием кода варианта.
