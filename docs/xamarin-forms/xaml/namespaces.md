...
Дополнительные сведения о `x:FieldModifier` атрибут, см. в разделе [модификаторы поля](~/xamarin-forms/xaml/field-modifiers.md). Дополнительные сведения о `x:Arguments`, `x:FactoryMethod`, и `x:TypeArguments` атрибутов, см. в разделе [передачи аргументов в XAML](~/xamarin-forms/xaml/passing-arguments.md).
