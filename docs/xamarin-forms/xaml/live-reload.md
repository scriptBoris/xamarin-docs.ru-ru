---
title: Динамической перезагрузки Xamarin (Предварительная версия)
...
Нет, предварительная версия Xamarin Live перезагрузки доступна только для Visual Studio 2017.
...
Динамической перезагрузки применяется только изменения, внесенные в XAML или CSS. При внесении изменений в файл C#, потребуется повторную компиляцию. 
...
* Повторная загрузка ресурсы на уровне приложения (т. е. **App.xaml** или Общие словари ресурсов), приложение навигации сбрасывается. 
* В настоящее время перезагрузки ContentView необходимо перезагрузить страницу.
