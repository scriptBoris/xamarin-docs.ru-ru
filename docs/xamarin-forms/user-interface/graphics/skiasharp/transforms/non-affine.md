...
Другой тип универсального неаффинные преобразования — это трехмерного поворота, как показано в следующей статье, [трехмерных вращений](~/xamarin-forms/user-interface/graphics/skiasharp/transforms/3d-rotation.md).

Неаффинных преобразование можно преобразовать прямоугольник в любой выпуклая четырехугольника осуществляется плавный переход. Это демонстрируется путем **Показать Non-аффинное** страницы. Это очень похоже на **Показать аффинное** странице из [матрица преобразует](~/xamarin-forms/user-interface/graphics/skiasharp/transforms/matrix.md) статьи, за исключением того, что он имеет четвертый `TouchPoint` для операций в четвертый углу точечного рисунка:
...
Другой неаффинные преобразования используется для имитации трехмерной графики. В следующей статье [трехмерных вращений](~/xamarin-forms/user-interface/graphics/skiasharp/transforms/3d-rotation.md) вы см. в разделе о смене двухмерной графики в трехмерном пространстве.
