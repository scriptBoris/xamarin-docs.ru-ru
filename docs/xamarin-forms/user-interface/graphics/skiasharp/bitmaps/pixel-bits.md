...
Это также интересно сравнить различные градиенты: верхних строк из всех трех платформах совпадают и Показать градиента, как предполагалось. Это означает, что `SetPixel` метод и `Pixels` свойство правильно создать пикселов из цветов, независимо от того, базовый формат пикселей.
...
Если вы хотите использовать тот же код для доступа к биты пикселов на всех трех платформах, можно явно создать `SKBitmap` одним `Rgba8888` или `Bgra8888` формат. Если вы хотите привести `SKColor` значения пикселов точечного рисунка, используют `Bgra8888`.
