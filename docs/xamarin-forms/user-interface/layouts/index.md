...
* **[StackLayout](stack-layout.md)**  — используется для упорядочивания представлений линейно, горизонтально или вертикально. Представления в StackLayout могут быть выровнены по центру, влево или вправо макета.
* **[AbsoluteLayout](absolute-layout.md)**  – используется для упорядочивания представлений, задав координаты, ра & змер с точки зрения абсолютные значения или соотношений. AbsoluteLayout может использоваться для уровня представления, а также привязывать их к слева, справа или center.
* **[RelativeLayout](relative-layout.md)**  — используется для упорядочивания представлений, задав ограничения относительно их родительского элемента размеры и положение.
* **[Сетка](grid.md)**  — используется для размещения представления в виде сетки. С точки зрения абсолютные значения или соотношений, можно указать строки и столбцы.
* **[FlexLayout](flex-layout.md)**  — используется для упорядочивания представлений по горизонтали или по вертикали при переносе.
* **[ScrollView](scroll-view.md)**  — используется для предоставления прокрутку, если представление не может поместиться полностью в пределах границ экрана.
* **[LayoutOptions](layout-options.md)**  — определить выравнивание и расширения для представления, относительно его родительского элемента.
* **[Входные данные прозрачности](#input_transparency)**  — указывает, является ли элемент получает входные данные.
* **[Поля и заполнение](margin-and-padding.md)**  — показано, как управлять поведением макета при подготовке к просмотру элемента в пользовательском интерфейсе.
* **[Ориентация устройства](device-orientation.md)**  — Описание способов обработки изменение ориентации устройства.
* **[Макет на устройства планшетных и настольных компьютеров](tablet.md)**  — показано, как оптимизация для больших экранов на каждой платформе.
* **[Привязываемые макеты](bindable-layouts.md)**  – включить классы макета для создания их содержимого путем привязки к коллекции элементов.
* **[Создание пользовательского макета](custom.md)**  — объясняется, как создать класс пользовательского макета.
* **[Сжатие макета](layout-compression.md)**  — Удаляет указанный макет из визуального дерева с целью повышения скорости отрисовки страниц.
...
`BindableLayout` Класс позволяет любой макет класса, производного от [ `Layout<T>` ](xref:Xamarin.Forms.Layout`1) класса, необходимо создать его содержимое, путем привязки к коллекции элементов, с параметром, чтобы задать внешний вид каждого элемента с [ `DataTemplate` ](xref:Xamarin.Forms.DataTemplate).
