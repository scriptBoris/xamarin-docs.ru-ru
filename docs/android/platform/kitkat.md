...
KitKat обеспечивает больший уровень контроля над темы приложения с помощью необязательно transclucent состояния и панели навигации. Вы можете изменить полупрозрачность элементов пользовательского интерфейса системы, в том же файле XML, используемого для определения темы Android. KitKat предоставляет следующие свойства:
...
[![Снимок экрана с примером приложения, используя платформу доступа хранилища для переход к изображению](kitkat-images/saf-ui.png)](kitkat-images/saf-ui.png#lightbox)
...
[![Пример снимка экрана вывод элемента меню печати в правом верхнем углу экрана](kitkat-images/menu.png)](kitkat-images/menu.png#lightbox)
...
(`null` в приведенном выше примере). Можно указать `PrintAttributes` для размещения содержимого на странице, несмотря на то, что атрибуты по умолчанию должен обрабатывать большинство scenarions.
...
Служба HCE также должна быть зарегистрирован в манифест приложения и с соответствующие разрешения, фильтр намерений и метаданных. Следующий код является примером `HostApduService` зарегистрирован с манифестом Android с помощью `Service` атрибут (Дополнительные сведения об атрибутах см. Xamarin [работа с манифестом Android](~/android/platform/android-manifest.md) руководство):
...
Можно создать `SensorManager` путем вызова `GetSystemService(SensorService)` и приведения результат в виде `SensorManager`. Чтобы использовать шаг счетчика, вызовите `GetDeafultSensor` на `SensorManager`. Вы можете зарегистрировать датчик и ожидания передачи изменений в число шагов, с помощью [`ISensorEventListener`](https://developer.xamarin.com/api/type/Android.Hardware.ISensorEventListener/)
...
-  *Чтение динамически изменение текста* -обозначают части пользовательского интерфейса, которые динамически обновить новым текстом «live» регионы с новым [ `accesibilityLiveRegion`](http://developer.android.com/reference/android/R.attr.html#accessibilityLiveRegion)
