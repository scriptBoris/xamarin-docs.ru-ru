...
**Создание пустой базы данных** &ndash; вызовите `CreateFile` метод с допустимый путь к файлу (т. е. записи). Стоит ли файл уже существует, перед вызовом этого метода, в противном случае поверх старого, в котором будет создан новый (пустой) базы данных и данных в старый файл, будут потеряны.
