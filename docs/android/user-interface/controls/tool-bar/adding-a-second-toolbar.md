...
`Toolbar` Можно сделать более чем заменить на панели действий &ndash; он может использоваться несколько раз в рамках действия, он может осуществляться настраивается для размещения в любом месте на экране, и его можно настроить охватывать только частичное ширину экрана. В приведенных ниже примерах показаны способы создания второй `Toolbar` и поместите его в нижней части экрана. Это `Toolbar` реализует **копирования**, **Вырезать**, и **вставить** пунктов меню. 
