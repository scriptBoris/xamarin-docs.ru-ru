...
2.  Настройка `NotificationChannel` объект первоначальной настройки.
...
В следующем образце кода показано, как создать базовое уведомление с помощью `NotificationCompat.Builder`:
