...
При использовании эмулятора пакетов SDK для Android от Google мы рекомендуем настроить его для использования аппаратного ускорения. Инструкции по настройке аппаратного ускорения см. в разделе [Аппаратное ускорение для производительной работы эмулятора](~/android/get-started/installation/android-emulator/hardware-acceleration.md).
