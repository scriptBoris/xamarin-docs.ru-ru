...
-   *Debug.Mono.Debug*: Если непустой строкой, это эквивалентно `*mono-debug*`.
...
-   *Debug.Mono.env*: отделенный вертикальной чертой ("*|*") список переменных среды для экспорта во время запуска приложения *перед* mono была инициализирована. Таким образом, задание переменных среды, управляющих mono ведением журнала.
    ...
    - *Примечание*: так как значение "*|*"-запятыми, значение должно содержать дополнительный уровень заключения в кавычки, как \` *оболочки adb* \` команды приведет к удалению набор кавычек.

    - *Примечание*: значения свойств системы Android может быть не длиннее 92 символов в длину.
...
-   *Debug.Mono.log*: разделенный запятыми ("*,*") список компонентов, которые должна вывести дополнительные сообщения в журнал отладки Android. По умолчанию имеет значение nothing. Компоненты:
    ...
    -   *сборщик мусора*: сообщения, относящиеся к печати GC.
...
Если вы не удалось активировать Xamarin.Android в Windows или не удалось установить полную версию Xamarin.Android на Mac OS X, см. статью [автономной активации](~/android/get-started/installation/index.md) страницы.
...
Если вы недавно приобрели Xamarin.Android и ранее работы с пробной версией Xamarin.Android, может потребоваться выполнить следующие действия, чтобы получить это изменение лицензии забирается Visual Studio для Mac или Visual Studio.

-  Закройте Visual Studio для Mac или Visual Studio
-  Удалите все файлы из ~/Library/MonoAndroid на компьютере Mac или %PROGRAMDATA%\Mono для Android\License\ для Windows
-  Снова откройте Visual Studio для Mac или Visual Studio и сборка проекта Xamarin.Android


Это должны помочь вам работу программ. Если вы проблемы продолжают возникать, можно попробовать [автономной активации](~/android/get-started/installation/index.md) для завершения активации рабочей станции.



## <a name="receiving-activation-incomplete-error-message"></a>Получение "неполные сообщение активации
...
Время от времени обновление завершится ошибкой из этой ошибки, который часто случается при поиске обновлений:

Большую часть времени, эта ошибка может быть решена просто выход из учетной записи Xamarin, а затем вернуть ведения журнала.

Для этого необходимо найти другой ниже платформе и следуйте инструкциям:
...
6. Проверить наличие обновлений

**На Компьютере, используя Visual Studo:**
1. Откройте Visual Studio
2. Выберите Инструменты > учетная запись Xamarin
3. Нажмите кнопку выхода
4. Нажмите кнопку входа
5. Введите свои учетные данные
6. Проверить наличие обновлений

Если это сообщение об ошибке продолжает появляться, обратитесь по электронной почте **contact@xamarin.com**.
...
Для решения этой проблемы, перейти в Visual Studio и изменить уровень детализации MSBuild, чтобы сделать это, выберите: **средства > Параметры > проекта** и **решений > Создание** и **запуска > Уровень детализации выходных данных сборки проекта MSBuild** и это значение равно **обычный**.
