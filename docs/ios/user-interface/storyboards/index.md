...
При создании раскадровки, с помощью конструктора, интегрированная среда разработки автоматически добавит [[регистрация]](https://developer.xamarin.com/api/type/Foundation.RegisterAttribute/) атрибут в верхней части `designer.cs` и передайте идентификатор строки, которое идентично идентификатора раскадровки, указанных в предыдущий шаг. Это свяжет C# в соответствующие сцену в раскадровке.
...
1. Теперь давайте создадим наш пользовательский интерфейс и добавить дополнительные представления, перечисленные в `MainViewController` чтобы это выглядело как тот, на следующем снимке экрана:
    ...
    - UILabel
        - Текст: "Ошибка: неправильный пароль. Не должен проходить! "
        - Цвета: красный
        - Выравнивание: Center
        - Строк: 2
        - Флажок «Скрытый» 
