...
Кроме того [UIKit](https://developer.xamarin.com/api/namespace/UIKit/) был изменен для работы в новом расширенных **sRGB** цветовое пространство, облегчая процесс смешивания цветов в охваты широкая цветовая палитра без потери производительности.

Apple предлагает следующие рекомендации при работе с шириной цвета:

- [UIColor](https://developer.xamarin.com/api/type/UIKit.UIColor/) теперь использует sRGB цветовое пространство и больше не будут clamp значения `0.0` для `1.0` диапазона. Если приложение зависит от предыдущего поведения выполняют приведение, его нужно будет изменить для iOS 10.
...
- `ADBannerView`, `ADInterstitialAd` и связанные с ней символы в [UIViewController](https://developer.xamarin.com/api/type/UIKit.UIViewController/) класс стали нерекомендуемыми.
