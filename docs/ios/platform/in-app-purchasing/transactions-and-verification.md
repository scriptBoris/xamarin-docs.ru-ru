...
6. Когда сервер получает запрос на содержимое, он анализирует данные приемки и отправляет запрос непосредственно к серверам iTunes для проверки поступления относится допустимой транзакции. Сервер должен использовать логику для определения, следует ли отправлять запрос в производственной или "песочницы" URL-адрес. Apple предлагает всегда с помощью URL-адреса производственной и переключение "песочницы", если ваш статус 21007 (поступление "песочницы" отправлено рабочий сервер). Ссылаться на Apple [руководство по программированию проверки поступления](https://developer.apple.com/library/archive/releasenotes/General/ValidateAppStoreReceipt/Chapters/ValidateRemotely.html) для получения дополнительных сведений.
...
В разделе Apple [руководство по программированию проверки поступления](https://developer.apple.com/library/archive/releasenotes/General/ValidateAppStoreReceipt/Introduction.html) документации для получения дополнительных сведений.
