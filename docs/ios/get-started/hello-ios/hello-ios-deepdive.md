...
- **Убирание клавиатуры** — когда пользователь касается текстового поля, iOS отображает клавиатуру, чтобы он мог ввести данные. К сожалению, встроенная функция для убирания клавиатуры отсутствует. Следующий код добавляется в `TranslateButton`, чтобы убрать клавиатуру, когда пользователь нажимает `TranslateButton`: PhoneNumberText.ResignFirstResponder (); еще один пример убирания клавиатуры см. в инструкциях по [убиранию клавиатуры](https://github.com/xamarin/recipes/tree/master/Recipes/ios/input/keyboards/dismiss_the_keyboard).
