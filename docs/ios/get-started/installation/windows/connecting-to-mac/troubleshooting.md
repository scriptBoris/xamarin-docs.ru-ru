---
title: Устранение неполадок подключения для узла сборки Xamarin.iOS
...
9. Если **шаг 8 завершился успешно**, можно попытаться выполнить простую команду, такую как `ls`, через подключение:
    ...
    Она должна вывести список содержимого домашнего каталога на компьютере Mac. Если команда `ls` работает правильно, но подключение к Visual Studio по-прежнему не удается установить, можно обратиться к разделу [Известные проблемы и ограничения](#knownissues) за сведениями о проблемах, характерных для Xamarin. Если вы не найдете описание, соответствующее возникшей проблеме, отправьте отчет об ошибке представителям сообщества разработчиков. Для этого последовательно выберите в Visual Studio пункты **Справка > Отправить отзыв > Сообщить о проблеме** и прикрепите журналы, описанные в разделе [Подробное ведение журнала](#verboselogs).
