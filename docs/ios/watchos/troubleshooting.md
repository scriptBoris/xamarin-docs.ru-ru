...
ms.locfileid: "56321159"
---
# <a name="watchos-troubleshooting"></a>watchOS Устранение неполадок

Эта страница содержит дополнительные сведения и возможные решения проблем, которые могут возникнуть.
...
- Не удается добавить два `WKNotificationControllers` раскадровки.
    Решение `notificationCategory` Элемент в раскадровке XML всегда вставляется с тем же `id`. Чтобы обойти эту проблему, можно добавить контроллеры уведомлений два (или более), откройте файл раскадровки в текстовом редакторе и вручную изменить `id` элемента должны быть уникальными.
...
5. Создайте еще один C# файл **MyInterfaceController.designer.cs** в **смотреть расширение приложения** проекта и добавьте приведенный ниже код. Не забудьте обновить пространство имен, имя класса и `Register` атрибут:
    ...
    Совет. (Необязательно) можно сделать этот файл дочерний узел первого файла, перетащив его на другой C# файл в Visual Studio для Mac панели решения. Оно затем будет выглядеть следующим образом:
