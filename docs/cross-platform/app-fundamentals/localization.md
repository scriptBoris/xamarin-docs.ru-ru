...
description: В этом документе описываются основные понятия кросс платформенных Интернационализация и локализация и анализирует их влияние на проектирование приложений.
ms.prod: xamarin
...
- [**Xamarin.Forms** ](~/xamarin-forms/app-fundamentals/localization/index.md) локализации с помощью RESX-файлы.
...
В разделе [разбор числовых строк](http://msdn.microsoft.com/library/xbtzcc4w(v=vs.110).aspx) и [строк синтаксического анализа дат и времени](http://msdn.microsoft.com/library/2h3syy57(v=vs.110).aspx) статьи MSDN для получения дополнительных сведений.
...
В разделе [подробное сравнение строк](http://msdn.microsoft.com/library/dd465121(v=vs.110).aspx#the_details_of_string_comparison) в [советы и рекомендации по использованию строк в .NET Framework](http://msdn.microsoft.com/library/dd465121(v=vs.110).aspx) пример, где порядок сортировки влияет на язык (CultureInfo).
